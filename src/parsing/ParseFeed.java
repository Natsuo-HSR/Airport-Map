...
public class ParseFeed {
	...
	public static List<PointFeature> parseMyAirports(PApplet p, String fileName) {
		List<PointFeature> features = new ArrayList<PointFeature>();
		...
		for (String row : rows) {
			...
			for (int s = 0; s < columns.length; s++) {
				columns[s] = columns[s].replaceAll(" ", "");
			}
			...
			// get location and create feature
			float lat = Float.parseFloat(columns[3]);
			float lon = Float.parseFloat(columns[4]);
	...
	public static List<ShapeFeature> parseMyRoutes(PApplet p, List<FlightModel> FlightModel) {
		List<ShapeFeature> routes = new ArrayList<ShapeFeature>();
		
		
		for(FlightModel f : FlightModel) {
			ShapeFeature route = new ShapeFeature(Feature.FeatureType.LINES);
			
			// set "source" property to be OpenFlights identifier for source airport
			//route.putProperty("source", f.originAirportId);
			route.putProperty("source", f.originCity);
			// "destination property" -- OpenFlights identifier
			//route.putProperty("destination", f.destAirportId);
			route.putProperty("destination", f.destCity);
	...
	public static List<ShapeFeature> parseMyRoutes2(PApplet p, String fileName) {
		List<ShapeFeature> routes = new ArrayList<ShapeFeature>();
		...
		for(String row : rows) {
			...
			for (int i = 0; i < columns.length; i++) {
                ...
                if (IsColumnPart(columns[i])) {
                    String lastText = columnList.get(columnList.size() - 1);
                    columnList.set(columnList.size() - 1, lastText + ","+ columns[i]);
                } else {
			...
			// set "source" property to be OpenFlights identifier for source airport
			route.putProperty("source", columnList.get(2));
			// "destination property" -- OpenFlights identifier
			route.putProperty("destination", columnList.get(4));
